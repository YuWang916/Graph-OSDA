...
class DomainData(InMemoryDataset):
    ...
    def process(self):
        ...
        label_to_remove = [0, 1]  # for 5 categories
        ...
        if self.domain == 'target':
            # unknown_class = min(label_to_remove)
            # y[y >= unknown_class] = unknown_class
            y_original = y.copy()
            unknown_class = 3
            y[y_original <= 1] = unknown_class
            y[y_original > 1] = y[y_original > 1] - 2
